...
class BaseActivity : AppCompatActivity() {
    ...
    private  val imageEditorLauncher = registerForActivityResult(ActivityResultContracts.StartActivityForResult()) { result ->
        if (result.resultCode == RESULT_OK) {
            Log.w(ContentValues.TAG, "Image editor launched successfully")
        }
    }
    ...
    private fun detectObjectsInImage(imgFile: File){
        ...
        objectDetector.process(image)
            ...
            .addOnFailureListener { e ->
                Log.d(ContentValues.TAG, "Failed to Detect")
            }
    }
    private fun drawBoundingBoxes(imgFile: File, objs: List<DetectedObject>){
        ...
        // Set color array
        val colorArr: IntArray = intArrayOf(
            Color.RED, Color.BLUE, Color.CYAN,
            Color.GREEN, Color.LTGRAY, Color.MAGENTA, Color.YELLOW, Color.DKGRAY)
